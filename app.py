...
def get_gold_data(days: int = 365) -> pd.DataFrame:
    ...
    try:
        ...
        # Friendly message for users without Polygon key
        if "api_key" in str(e).lower() or "authentication" in str(e).lower() or "401" in str(e):
            st.warning("Gold data requires a free Polygon.io API key. Get one at https://polygon.io/ and set POLYGON_API_KEY environment variable, or the Gold tab will show limited info.")
        else:
            st.info("Gold data temporarily unavailable. Crypto tabs are fully working.")
        return pd.DataFrame()

@st.cache_data(ttl=600, show_spinner=False)
def get_crypto_ohlcv(coin_id: str, days: int = 200) -> pd.DataFrame:
    """Fetch OHLCV + volume from CoinGecko public API (more reliable for deployment)"""
    try:
        # OHLC data
        ...
        # Volume data (separate endpoint)
        try:
            vol_url = f"https://api.coingecko.com/api/v3/coins/{coin_id}/market_chart"
            vol_resp = requests.get(vol_url, params={"vs_currency": "usd", "days": str(days)}, timeout=10)
            vol_resp.raise_for_status()
            vol_json = vol_resp.json()
            if "total_volumes" in vol_json and vol_json["total_volumes"]:
                vol_df = pd.DataFrame(vol_json["total_volumes"], columns=["timestamp", "volume"])
...
def get_coin_market_data(coin_id: str) -> dict:
    """Get current market stats from CoinGecko public API"""
    try:
        ...
        return {
            ...
            "total_volume": m.get("total_volume", 0) or 0,
            "price_change_24h": m.get("price_change_percentage_24h", 0) or 0,
            "market_cap_rank": m.get("market_cap_rank"),
            "name": m.get("name", coin_id),
            "symbol": (m.get("symbol") or coin_id).upper()
        }
    except Exception:
        return {}
...
def get_pump_fun_new_launches(limit: int = 12, sort: str = "created_timestamp") -> list:
    """Fetch newest or trending launches from Pump.fun (public frontend API)"""
    try:
        url = "https://frontend-api.pump.fun/coins"
        params = {
            "offset": "0",
            "limit": str(limit),
            "sort": sort,           # created_timestamp or last_trade_timestamp or market_cap
            "order": "DESC",
            "includeNsfw": "false"
        }
...
with tab_meme:
    ...
    try:
        ...
        for m in top_meme_markets:
            ...
            if price > 0:
                meme_overview.append({
                    "Name": f"{m.get('name', 'Unknown')} ({(m.get('symbol') or '').upper()})",
                    "Price": price,
                    "24h %": round(m.get("price_change_percentage_24h") or 0, 1),
                    "Market Cap": m.get("market_cap") or 0
                })
        if meme_overview:
        ...
        st.info("Could not load live top memecoins list right now. The detailed selector above still works for popular ones.")
    
    # ==================== NEW: PUMP.FUN NEW LAUNCHES ====================
    st.divider()
    with st.expander("🚀 New & Trending on Pump.fun (Latest Launches)", expanded=True):
        st.caption("Fresh memecoins launching on Solana. Data updates every ~2 minutes. Use DexScreener/Birdeye for live charts & liquidity.")
        
        col1, col2 = st.columns([1, 1])
        with col1:
            sort_option = st.selectbox(
                "Sort by",
                ["Newest (created)", "Recently traded", "Highest market cap"],
                index=0,
                key="pump_sort"
            )
        
        sort_map = {
            "Newest (created)": "created_timestamp",
            "Recently traded": "last_trade_timestamp",
            "Highest market cap": "market_cap"
        }
        selected_sort = sort_map[sort_option]
        
        if st.button("🔄 Refresh Pump.fun Data", key="refresh_pump"):
            st.cache_data.clear()
            st.rerun()
        
        pump_data = get_pump_fun_new_launches(limit=10, sort=selected_sort)
        
        if pump_data:
            pump_table = []
            for coin in pump_data[:10]:
                mint = coin.get("mint", "")
                ...
                if created:
                    try:
                        age_min = (datetime.now().timestamp() * 1000 - created) / 60000
                        if age_min < 60:
                ...
                pump_table.append({
                    "Name": f"{coin.get('name', 'Unknown')} ({coin.get('symbol', '').upper()})",
                    "Market Cap": f"${mc:,.0f}" if mc else "N/A",
                    "Age": age_str,
                    "Complete": "✅ Raydium" if coin.get("complete") else "⏳ Bonding"
                })
            
            pump_df = pd.DataFrame(pump_table)
            st.dataframe(pump_df, use_container_width=True, hide_index=True)
            
            st.markdown("**Quick Links for these launches:**")
            link_cols = st.columns(3)
            for i, coin in enumerate(pump_data[:6]):
                mint = coin.get("mint", "")
                name = coin.get("name", "coin")
                with link_cols[i % 3]:
                    if mint:
                        st.link_button(f"🔗 {name[:15]} on DexScreener", f"https://dexscreener.com/solana/{mint}", use_container_width=True)
                        st.link_button(f"🐦 {name[:15]} on Pump.fun", f"https://pump.fun/{mint}", use_container_width=True)
                        st.link_button(f"👁️ Birdeye", f"https://birdeye.so/token/{mint}?chain=solana", use_container_width=True)
        else:
            st.warning("Could not fetch latest Pump.fun launches right now (rate limit or temporary issue). Try again in a minute.")
